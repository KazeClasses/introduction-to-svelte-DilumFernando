<script>
	...
	function updateBirthrate(index, value) {
		points = points.map((point, i) =>
			i === index ? { ...point, birthrate: +value } : point
		);
	}
</script>
...
<div>
	{#each points as point, i}
		<label>
			{point.year} Birthrate:
			<input
				type="number"
				bind:value={point.birthrate}
				step="0.1"
				min="0"
				on:input={(e) => updateBirthrate(i, e.target.value)}
			/>
		</label><br />
	{/each}
</div>
